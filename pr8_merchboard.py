# Pandas Analyzer & Visualization – variant 12
import os, sys
import numpy as np
import pandas as pd
...
class MerchBoard:
    ...
    def merge_in(self, other_df):
        if self.data.empty: return
        return pd.concat([self.data, other_df], ignore_index=True)

    def cut_by(self, by_col="Region"):
        if self.data.empty: return {}
        groups = {}
    ...
    def query_filter(self, keyword="A", sort_by="Sales", min_val=0):
    ...
    def plotter(self):
        ...
        plt.savefig("plot_bar_12.png"); plt.close()

        plt.figure()
        self.data.plot(kind="scatter", x="Year", y="Sales")
        plt.title("Scatter"); plt.savefig("plot_scatter_12.png"); plt.close()

        plt.figure()
        sns.boxplot(data=self.data[["Sales","Profit"]]); plt.title("Box Plot")
        plt.savefig("plot_box_12.png"); plt.close()
...
def main():
    ...
    while True:
        ...
        if ch == "1":
            path = input("CSV path (or new path to auto-generate): ") or "sales_12.csv"
            tool.fetch_csv(path); print("Loaded.")
        elif ch == "2":
            tool.outline()
        elif ch == "3":
            tool.neaten(); print("Cleaned.")
        elif ch == "4":
            print("Sum of first five (demo):", tool.mathbits())
        elif ch == "5":
            print(tool.aggset())
        elif ch == "6":
            print(tool.statlab())
        elif ch == "7":
            print(tool.pivot_make())
        elif ch == "8":
            tool.plotter(); print("Saved plots.")
        elif ch == "9":
            print("Bye"); break
        else:
            print("Invalid")
